...
void Start()
{
    // In your ArcadeKart Start() method, add this:
    if (gameObject.CompareTag("Player") || gameObject.CompareTag("Human"))
    {
        // Reduce speed for human players
        m_FinalStats.TopSpeed *= 0.85f;        // 15% speed reduction
        m_FinalStats.Acceleration *= 0.9f;   // 10% acceleration reduction

        m_FinalStats.Grip *= 0.9f;          // 10% traction/grip reduction
    }
}
